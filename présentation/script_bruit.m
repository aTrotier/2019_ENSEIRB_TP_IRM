...
p=readParams_Bruker('dirPath','/Users/aurelien/Documents/Dev/TP_IRM/TP_IRM/data/noise/AT_CoursNoise_01_1_1/3');

[im,im2,rawData]=recoCart(p);

dataMag=abs(double(reshape(im2(:,:,:,:,1),[],1)));
dataReal=real(double(reshape(im2(:,:,:,:,1),[],1)));
dataImag=imag(double(reshape(im2(:,:,:,:,1),[],1)));
...
A=0;
for i=1:4
    dataMagMc=squeeze(sqrt(sum(abs(double(im2(:,:,:,:,1:i))).^2,5)));
