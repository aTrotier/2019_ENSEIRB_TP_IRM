...
%modif kspace 1
kspace1=kspace;
kspace1(90,169)=kspace1(90,169)*200;
...
%modif kspace 3
kspace3=kspace;
kspace3(end/2+1,end/2+1+60)=kspace3(end/2+1,end/2+1+60)*500;
imModif3=fft2(kspace3);

%modif kspace 4
kspace4=kspace;
kspace4(end/2+1+60,end/2+1)=kspace4(end/2+1+60,end/2+1)*500;
imModif4=fft2(kspace4);
...
% 
% subplot(3,3,4); imshow(abs(kspace),[mini,maxi]);
% subplot(3,3,5); imshow(abs(kspace1),[mini,maxi]);
% subplot(3,3,6); imshow(abs(kspace2),[mini,maxi]);
% 
...
%
imagine(abs(kspace),abs(kspace1),abs(kspace2),abs(kspace3),abs(kspace4),...
    abs(imSOS),abs(imModif1),abs(imModif2),abs(imModif3),abs(imModif4));
